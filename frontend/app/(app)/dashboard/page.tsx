    ...
    <>
      ...
      <main className="p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <button className="flex items-center justify-between bg-blue-600 hover:bg-blue-700 text-white rounded-xl p-5 transition-colors">
            <div className="flex items-center gap-3">
              <LinkIcon className="w-5 h-5" />
              <div className="text-left">
                <p className="font-semibold">Join Meeting</p>
                <p className="text-sm text-blue-100">Enter a meeting link to join instantly</p>
              </div>
            </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 mb-6">
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            ...
            <Link href="/meetings" className="text-sm text-blue-600 font-medium flex items-center gap-1 hover:text-blue-700">
              View All <ChevronRight className="w-4 h-4" />
            </Link>
          </div>

          {loading ? (
            <div className="py-12 text-center text-slate-400 text-sm">Loading meetings...</div>
          ) : meetings.length === 0 ? (
            <div className="py-12 text-center">
              <p className="text-slate-500 mb-4">No meetings yet</p>
              <Link href="/meetings" className="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition-colors">
                <Plus className="w-4 h-4" />
                  ...
                  {meetings.map((meeting) => (
                    <tr key={meeting.id} className="border-b border-slate-50 last:border-0">
                      <td className="pl-6 pr-2 py-4">
                        <input type="checkbox" className="w-4 h-4 rounded border-slate-300 text-blue-600" checked={selected.has(meeting.id)} onChange={() => toggleSelected(meeting.id)} />
                      </td>
                      <td className="px-2 py-4">
                        <button onClick={() => toggleFavorite(meeting.id)} className={favorites.has(meeting.id) ? 'text-amber-400' : 'text-slate-300 hover:text-amber-400'}>
                          <Star className="w-4 h-4" fill={favorites.has(meeting.id) ? 'currentColor' : 'none'} />
                        </button>
                      </td>
                      <td className="px-2 py-4 text-slate-600 whitespace-nowrap">{formatDate(meeting.created_at)}</td>
                      <td className="px-2 py-4">
                        <p className="font-medium text-slate-900">{meeting.title}</p>
                        <p className="text-slate-400 text-xs truncate max-w-xs">{meeting.description}</p>
                      </td>
                      <td className="px-2 py-4 text-slate-500 whitespace-nowrap">{formatDuration(meeting.duration_minutes)}</td>
                      <td className="px-2 py-4">
        ...
        <div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl p-8 border border-blue-100">
          <h2 className="text-xl font-bold text-slate-900 mb-2">Turn Conversations into Real Progress</h2>
          <p className="text-slate-600 mb-4 max-w-lg">
            MeetMate helps you capture, summarize and act on what matters — so you can focus on what&apos;s next.
          </p>
...
function StatCard({ icon, iconBg, label, value, change, changeSuffix }: { icon: React.ReactNode; iconBg: string; label: string; value: string; change: number; changeSuffix: string }) {
